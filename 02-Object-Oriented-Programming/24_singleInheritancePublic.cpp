...
class base{
    ...
    public:
        int data2;
        void setData(void);
        int getData1();  //since we cant access private member form inherited class(derived) so we declared a function that is public whichcan manipulate priavte member of base class
        int getData2();
};
...
int main(){ 
    ...
    derived val;
    val.setData(); //---> we can use base class public function in our derived class since it is inherited publically
    val.process();
    val.display();
